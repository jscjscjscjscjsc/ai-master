...
(function() {
  ...
  // 这句话是原版的灵魂，保留为默认值；但跃迁到不同章节时，
  // 换一句贴题的引言比每章都说同一句更合适 —— 所以允许宿主覆盖。
  const _cfg = window.STARLAB_TRANSITION || {};
  const QUOTE = _cfg.quote || '真正的大师永远怀着一颗学徒的心';
  const CHAR_INTERVAL = _cfg.charInterval || 130; // ms per character
  ...
  function startRedirectTimer() {
    // 原版把目的地和时长写死了（5 秒后硬跳 ../../dashboard），
    // 而这一页现在被平台复用为「章节之间的跃迁」：目的地要能指定，
    // 时长要能调，验收时还要能停住不跳。
    // 配置来自宿主页面写的 window.STARLAB_TRANSITION，未配置时退回原行为。
    const cfg = window.STARLAB_TRANSITION || {};
    const total = Number(cfg.duration) > 0 ? Number(cfg.duration) : 5000;
    const fadeAt = Math.max(600, total - 700);
    const target = cfg.to || '../../dashboard';
    ...
    setTimeout(() => {
      const overlay = document.querySelector('.transition-overlay, #tp-overlay')
        || document.getElementById('tp-overlay');
