...
const Cultivation = {
  ...
  storageKey: 'starlab_cult_collapsed',

  init() {
    // 默认**收起**。展开态约 218×230px，固定在左下角，会压住正文左下区域
    // （内容左边缘 140px < 悬浮条右边缘 234px），而且是每一页都压。
    // 收起后是一枚 56px 的境界法印：信息还在，但不挡任何内容。
    // 用户主动展开过就记住他的选择。
    let saved = null;
    try { saved = localStorage.getItem(this.storageKey); } catch (e) { saved = null; }
  ...
  render(options = {}) {
    ...
    dock.addEventListener('click', (event) => {
      ...
      // 点法印/名字：能开等级详情就开，否则去星空修为页
      if (event.target.closest('.cult-head')) this.openHere();
    });
  },

  /** 有星空修为脚本就弹等级详情，没有就跳页——两种都是可用的正反馈。 */
  openHere() {
    const profile = this.profile;
    if (!profile) return;
  ...
  applySettlement(settlement) {
    ...
    if (before && typeof profile.equipment_unlocked === 'number'
      ...
      this.toast(`装备解锁 +${delta}`, '去星空修为页看是哪一件', 4200);
    }
  },
