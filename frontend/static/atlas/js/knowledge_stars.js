(() => {
  ...
  /* 星球/星系名标签的定位。
     左栏（标题与统计）和底栏（操作提示）都是 HUD，标签不能压上去 ——
     压在文字上两行都读不清。所以越界时把标签翻到光标另一侧，而不是硬挤。 */
  function placeLabel(el, event) {
    const pad = 14;
    const hudLeft = document.querySelector('.atlas-panel');
    const hudLeftRight = hudLeft ? hudLeft.getBoundingClientRect().right : 0;
    const w = el.offsetWidth || 180;
    const h = el.offsetHeight || 28;
    let x = event.clientX + 13;
    let y = event.clientY - h / 2;
    // 右侧越界 → 翻到左侧
    if (x + w > innerWidth - pad) x = event.clientX - w - 13;
    // 左侧压到 HUD → 再往右让开
    if (x < hudLeftRight + pad) x = hudLeftRight + pad;
    // 上下越界
    if (y < pad) y = pad;
  ...
  function updateHover(event) {
    ...
    if (activeGalaxy && next?.userData?.hitType === "planet") {
      label.textContent = next.userData.star.title; placeLabel(label, event); label.classList.add("show"); canvas.style.cursor = "pointer";
    } else if (!activeGalaxy && next?.userData?.hitType === "galaxy") {
      label.textContent = next.userData.galaxy.name; placeLabel(label, event); label.classList.add("show"); canvas.style.cursor = "pointer";
  ...
  async function loadUniverse() {
    ...
    try {
      const _cfg = window.STARLAB_ATLAS || {};
      const response = await fetch(_cfg.universeUrl || "../data/knowledge-universe.json",
  ...
  function initControls() {
    ...
    backButton.addEventListener("click", backToUniverse); document.querySelector("#homeButton").addEventListener("click", backToUniverse); document.querySelector("#dashboardButton").addEventListener("click",
    () => location.assign((window.STARLAB_ATLAS || {}).dashboardUrl || "../../dashboard")); document.querySelector("#panelClose").addEventListener("click", () => { selectedPlanet = null; panel.classList.remove("show"); updateFocus(); }); document.querySelector("#retryButton").addEventListener("click", () => { errorCard.classList.remove("show"); paused = false; loadUniverse(); });
  ...
  try { createRenderer(); resize(); initControls(); loadUniverse(); animate(); } catch (error) { console.error(error); errorMessage.textContent = "浏览器未能初始化 WebGL 星图。"; errorCard.classList.add("show"); loading.classList.add("hide"); }
})();
